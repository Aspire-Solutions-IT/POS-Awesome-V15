					...
					<v-window v-model="activeTab">
						...
						<v-window-item value="drafts">
							...
							<div v-else-if="!filteredDraftInvoices.length" class="empty-state">
								...
								<div class="empty-state__subtitle">
									{{ __("Saved draft invoices will appear here.") }}
								</div>
							</div>
								...
								<template #item.actions="{ item }">
									<div class="d-flex justify-end ga-1">
										<v-btn
											...
											color="primary"
											:title="__('Load Draft')"
											:aria-label="__('Load draft invoice')"
...
export default {
	...
	methods: {
		...
		async loadDrafts() {
			if (!this.posOpeningShift?.name && !this.isSupervisorScope())
				return void (this.draftInvoices = []);
			this.loading = true;
			try {
				const { message } = await frappe.call({
				...
				this.draftInvoices = Array.isArray(message)
					? message.map((entry) => ({
							...entry,
							doctype: entry.doctype || this.currentInvoiceDoctype,
						}))
					: [];
			} catch (error) {
